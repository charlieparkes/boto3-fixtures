...
def test_to_dict(fixture_name, fixture):
    ...
    if raises:
        with pytest.raises(raises):
            output = boto3_fixtures.contrib.boto3._to_dict(fixture["input"])
    else:
        output = boto3_fixtures.contrib.boto3._to_dict(fixture["input"])
...
def test_client(fixture_name, fixture, service_name, environment):
    ...
    with utils.set_env(env):
        client = boto3_fixtures.contrib.boto3.client(service_name, **fixture)
        if endpoint_url:
...
def test_resource(fixture_name, fixture, service_name, environment):
    ...
    if endpoint_url:
        env["AWS_ENDPOINTS"] = service_name + "=" + endpoint_url
    with utils.set_env(env):
        resource = boto3_fixtures.contrib.boto3.resource(service_name, **fixture)
