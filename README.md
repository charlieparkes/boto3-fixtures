...
**boto3_fixtures** provides a simple set of tools for writing clearly defined, multi-function AWS Lambdas in Python.

This project was borne out of a desire to support directed-graph workflows on FAAS. **boto3_fixtures** is designed to handle batched events from CloudWatch, Kinesis, or SQS.
...
def lambda_handler(event, context):
    return boto3_fixtures.process_event(
        event=event,
        context=context,
        paths={
            "EXAMPLE": [test_func]
        },
        queue_type=boto3_fixtures.QueueType.SQS,
    )
```
...
You may run into a situation where you'd like to trigger a lambda with a message that can't conform to the boto3_fixtures message format but would still prefer to use boto3_fixtures for all it's boilerplate.
...
`boto3_fixtures` relies on exceptions for flow control. Your code must raise exceptions that inherit from one of two classes.
...
from boto3_fixtures import Action
...
`boto3_fixtures` can also generate the enumeration for you automatically...
...
```python
boto3_fixtures.Action(required_params, functions, paths)
```
...
```python
boto3_fixtures.Queue(type, name, path)
```

| Argument          | Type | Description                     |
| ----------------- | ---- | ------------------------------- |
| `type` | `boto3_fixtures.pipeline.QueueType` | |
...
```python
from boto3_fixtures import Queue, QueueType
